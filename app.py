    ...
    try:
        ...
        if match_check or alt_match_check:
            ...
            if match_check:
                ...
                if dropdown_value == 'option4':
                    cursor.execute(query_table, (threshold, max_players, min_players, min_year, max_year, num_results))
                    data_list = cursor.fetchall()
                    cursor.close()
                    random.shuffle(data_list)
                    data_list = data_list[:16]
                else:
                    cursor.execute(query_table, (threshold, max_players, min_players, min_year, max_year, suggestions))
                    data_list = cursor.fetchall()
                    cursor.close()
            ...
            no_game = [1]
            return render_template('index_2.html', search_name=search_name, feedback=no_game, top_suggest=top_suggest,
                                   two_player_suggest=two_player_suggest,
                                   party_suggest=party_suggest,
                                   min_players=min_players, max_players=max_players,
                                   min_year=min_year, max_year=max_year,
...
def send_email():
    ...
    msg = Message('Hello from ' + name_value, sender='reply.wswp@gmail.com', recipients=['tdklaus@gmail.com',
                                                                                         'abigailmklaus@gmail.com',
                                                                                         email_value])
    msg.body = message_value
    mail.send(msg)
    return 'Email sent!'
